...
$kodes = [];
if (isset($_GET['kode']) && is_array($_GET['kode'])) {
  $kodes = $_GET['kode'];
} elseif (isset($_GET['kodes'])) {
  $kodes = array_filter(array_map('trim', explode(',', $_GET['kodes'])));
...
// window: from client time to +15 minutes
$start_dt = $dt;
$end_dt = $dt->modify('+15 minutes');
